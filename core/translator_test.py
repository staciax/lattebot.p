...
class Translator(app_commands.Translator):
    ...
    async def translatex(self, string: locale_str, locale: Locale, context: TranslationContext) -> Optional[str]:
        localizable: Localizable = context.data
        tcl: TCL = context.location

        if tcl in [TCL.command_name, TCL.group_name] and isinstance(localizable, (Command, Group)):
    ...
    async def translate(self, string: locale_str, locale: Locale, context: TranslationContext) -> Optional[str]:
        ...
        # if binding is None:
        #     print(localize_keys)
        # cogs = self.bot.get_cog('cogs.' + binding.lower() if binding is not None else '')
        # print(cogs)

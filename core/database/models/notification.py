...
class Notification(Base):
    ...
    @classmethod
    async def read_all_by_owner_id_and_type(cls, session: AsyncSession, owner_id: int, type: str) -> AsyncIterator[Self]:
    ...
    async def create(cls, session: AsyncSession, owner_id: int, item_id: str, type: str) -> Self:
        ...
        session.add(notify)
        await session.flush()
        new = await cls.read_by_id(session, notify.id)
        if not new:
            raise RuntimeError()
        return new

    @classmethod
    async def delete(cls, session: AsyncSession, notify: Self) -> None:
        stmt = delete(cls).where(cls.id == notify.id)
    ...
    @classmethod
    async def delete_by_owner_id_and_item_id(cls, session: AsyncSession, owner_id: int, item_id: str) -> None:
        stmt = delete(cls).where(cls.owner_id == owner_id).where(cls.item_id == item_id)
        await session.execute(stmt)
        await session.flush()
